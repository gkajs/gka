...
function gka(dir, opt) {
    ...
    var dest = (opt.output && opt.output !== true)? opt.output: path.join(dir, "..", "gka-" + (cmd === "tool"? 'tool-': "") + path.basename(dir) + (isCrop? '-c': "") + (isUnique? '-u': "") + (isSprites? '-s': "") + (tpl? ('-'+ tplName): "") + (prefix? ('-'+ prefix): ""));
