...
function gka(dir, opt) {
    ...
    var dest = (opt.output && opt.output !== true)? opt.output: path.join(dir, "..", "gka-" + (cmd === "tool"? 'tool-': "") + path.basename(dir) + (isDiff? '-d': "") + (bgColor? ('-' + bgColor): "") + (isCrop? '-c': "") + (isUnique? '-u': "") + (isSprites? '-s': "") + (spritesCount? ('-'+ spritesCount): "") + (tpl? ('-'+ tplName): "") + (prefix? ('-'+ prefix): ""));

    dest = cfg.output !== undefined? cfg.output: util.getRealDest(dest);
