...
function sprites(obj) {
    var res = obj.sprites;
    // var res = {}, coordinates = obj.coordinates;
    // for(var p in coordinates){
    //     res[path.basename(p)] = coordinates[p];
    // }
var text = 
`/* 合图信息
* 
...
var sprites = ${JSON.stringify(res, null, '    ')}
`;
return text;
}
...
function cut(obj) {
    var res = {}, 
        ...
        // console.log(src2dist)

        // res = {
        //    src2cutinfo:src2cutinfo, 
        //    src2dist:src2dist, 
        // }
    for(var p in src2cutinfo){
        res[path.basename(src2dist[p])] = src2cutinfo[p];
    }
...
var cut = ${JSON.stringify(res, null, '    ')}
`;
return text;
}
...
var names = ${JSON.stringify(obj.names)}
`;
return text;
}
