...
function getData(obj) {
    var src2cutinfo = obj.src2cutinfo || {},
        ...
        src2splitInfo = obj.src2splitInfo,
        res = {},
        frames = [];

    if (src2splitInfo) {
        for(var src in src2splitInfo) {
            var splitInfos = src2splitInfo[src] || {};
            var cutInfo = src2cutinfo[src] || {};

            var _splitFiles = [];
          
            for (var i = 0, info; i < splitInfos.length; i++) {
                info = splitInfos[i];

                var subfile = info.subfile;

                var spritesInfo = sprites.frames && sprites.frames[path.basename(subfile)] || {};

                _splitFiles.push({
                    // "name": path.basename(src),
                    "file": spritesInfo.file || sprites.file || path.basename(src2dist[subfile]), // 当前图片名
                    "x": spritesInfo.x,        // 当前图片坐标
                    "y": spritesInfo.y,
                    "offX": info.offX,
                    "offY": info.offY,
                    "width": info.width || cutInfo.w || sizeOf(subfile).width,  // 当前图片宽度
                    "height": info.height || cutInfo.h || sizeOf(subfile).height,
                    "sourceW": info.sourceW || cutInfo.sourceW, // 原图片宽高，不 cut 时等于当前图片宽高 则省略
                    "sourceH": info.sourceH || cutInfo.sourceH,
                    "w": spritesInfo.w,
                    "h": spritesInfo.h,
                })
            }
            frames.push(_splitFiles);
        }
    } else {
